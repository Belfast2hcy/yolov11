...
def process_yolo_model(input_dir, model_file):
    model = YOLO(model_file)  # 直接加载.pt模型
    results = {}
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.jpg', '.png')):  # 确保不区分大小写
            ...
            try:
                detections = model.predict(file_path)  # 预测
    ...
    for key, value in results.items():
        ...
        for item in value:
            ...
            for row in range(len(box.xyxy)):
                ...
                ET.SubElement(bndbox, "confidence").text = str(float(box.conf[row]))  # 直接使用浮点数
...
def main():
    ...
    parser.add_argument('-model', required=True, help='YOLO model file path (.pt)')
